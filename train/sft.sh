...
torchrun --nproc-per-node ${gpu_count} --master_port 12345 \
    ...
    --activation_checkpointing=True # Enable gradient checkpointing for efficient memory usage with 8 H100 GPUs.
    # --gradient_checkpointing=True \ Reduce memory usage further by offloading activations to CPU when using gradient checkpointing.
